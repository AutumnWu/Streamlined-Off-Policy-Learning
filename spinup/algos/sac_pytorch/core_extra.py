...
class Mlp_unsup(nn.Module):
    def __init__(
        ...
        ## decode layer will output observation
        self.decode_layer = nn.Linear(in_size, decode_size)
        self.decode_layer.weight.data.uniform_(-init_w, init_w)
        self.decode_layer.bias.data.uniform_(-init_w, init_w)
    ...
    def forward(self, input):
        ...
        output = self.last_fc_layer(h)
        return output
    def autoencoder_forward(self, input): ## use this to reconstruct observation state
        h = input
        for i, fc_layer in enumerate(self.hidden_layers):
            h = fc_layer(h)
            h = self.hidden_activation(h)
        output = self.decode_layer(h)
        return output


class TanhGaussianPolicy_unsup(Mlp_unsup):
    def __init__(
            self,
            obs_dim,
            action_dim,
    ...
    def forward_both(
        ...
        for fc_layer in self.hidden_layers:
            h = self.hidden_activation(fc_layer(h))
        reconstruction_output = self.decode_layer(h)
        mean = self.last_fc_layer(h)
        ...
        if deterministic:
            ...
            if return_log_prob:
                ...
                if reparameterize is True:
                    ...
                    action = tanh_normal.sample()
        return (
            action * self.action_limit, mean, log_std, log_prob, std, pre_tanh_value, reconstruction_output
        )
